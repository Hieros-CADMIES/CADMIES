...
from paths import SOURCE_CONCEPTS_DIR, INDEX_FILE, BLOCKS_DIR
...
def enrich_concept(concept, model=MODEL):
    ...
    if not gaps:
        print(f"  CLEAN: {human_id}: No gaps detected. Skipping.")
        return None
    
    print(f"  GAPS: {human_id}: {', '.join(gaps)}")
    ...
    try:
        ...
        print(f"  OK: {human_id}: Enrichment successful")
        return enriched
        
    except json.JSONDecodeError:
        print(f"  FAILED: {human_id}: JSON parse failed on enrichment response")
...
def validate_and_remint(concept, dry_run=False):
    ...
    if not is_valid:
        print(f"  FAILED: {human_id}: Validation failed after enrichment")
        for e in errors:
            print(f"     - {e}")
        return {"success": False, "human_id": human_id, "error": "validation_failed", "details": errors}
    
    if dry_run:
        print(f"  DRY RUN: {human_id}: Would remint here")
    ...
    try:
        ...
        print(f"  WARNING: {human_id}: Provenance creation failed — {e}")
    
    print(f"  REMINTED: {human_id}: v{concept['metadata']['version']} | CID: {cid_result['cid'][:30]}...")
    if old_cid:
...
def main():
    ...
    print("=" * 60)
    print(f"CADMIES CONCEPT ENRICHMENT PIPELINE v{VERSION}")
    print(f"Model: {MODEL}")
    ...
    for i, human_id in enumerate(human_ids):
        ...
        if not concept:
            print(f"  WARNING: Source concept not found: {human_id}.json")
            continue
